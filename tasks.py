...
from agents import coordinator_agent, travel_info_agent, local_recommendation_agent, kakao_maps_agent
...
# 3단계: 카카오 맵을 활용한 장소 검색 및 추천 (kakao_maps_agent)
kakao_maps_task = Task(
    description=(
        "2단계에서 조회된 장소를 카카오 맵 API를 활용하여 지도와 상세 정보를 검색합니다."
    ),
    expected_output="카카오 맵 API를 통해 검색된 장소의 지도와 상세정보",
    agent=kakao_maps_agent,
    context=[local_recommendation_task]
)

# 4단계: 전체 일정 최종 정리 (coordinator_agent)
