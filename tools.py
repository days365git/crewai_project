...
# KakaoMapsTool
class KakaoMapsInput(BaseModel):
    query: str = Field(..., description="검색할 장소명")
    radius: int = Field(1000, description="검색 반경(미터 단위)")

class KakaoMapsTool(BaseTool):
    name: str = "카카오 맵 도구"
    description: str = "카카오 맵 API를 사용하여 장소 검색 및 상세 정보를 조회합니다."
    args_schema: Type[BaseModel] = KakaoMapsInput

    def _run(self, query: str, radius: int = 1000):
        location = self.get_location_by_query(query)
        nearby_places = self.find_nearby_places(location, radius)
        detailed_places = [self.get_place_details(place['place_name']) for place in nearby_places]
        return detailed_places

    def get_location_by_query(self, query: str):
    ...
    def get_place_details(self, place_name):
        ...
        if response["documents"]:
            ...
            return {
                ...
                "전화번호": place.get("phone", "정보 없음"),
                "카테고리": place.get("category_name"),
                "링크": place.get("place_url")
            }
        else:
            raise Exception("장소 세부 정보를 찾을 수 없습니다.")