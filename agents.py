...
from tools import FlightSearchTool, HotelSearchTool, NearbyPlacesTool, ExchangeRateTool, KakaoMapsTool
...
kakao_maps_agent = Agent(
    role="카카오 맵 전문가",
    goal="카카오 맵 API를 활용하여 고객이 요청한 장소의 상세 정보와 주변 추천 명소를 제공합니다.",
    backstory="당신은 카카오 맵 API를 활용하여 장소 검색 및 추천 정보를 제공하는 전문가입니다. "
              "고객이 요청한 장소의 상세 정보(주소, 전화번호, 웹사이트 등)를 정확히 제공하며, "
              "주변의 추천 명소를 추가로 제안합니다.",
    tools=[
        KakaoMapsTool()
    ],
    verbose=True
